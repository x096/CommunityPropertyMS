...
public class PositionMngDlg extends JFrame {
	...
	protected void updateActionPerformed(ActionEvent evt) {
		...
		if(StringUtil.isEmpty(positionnote)){
			JOptionPane.showMessageDialog(null, "职位备注不能为空！");
			return;
		}
		...
		try{
			...
			if(addNum==1){
				JOptionPane.showMessageDialog(null, "职位信息修改成功！");
				resetValue();
				this.fillTable(new Position());
			}else{
				JOptionPane.showMessageDialog(null, "职位信息修改失败！");
			}
		}catch(Exception e){
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "职位信息修改失败！！");
	...
	protected void addActionPerformed(ActionEvent ae) {
		...
		try{
			...
			if(addNum==1){
				JOptionPane.showMessageDialog(null, "职位信息添加成功！");
				resetValue();
				this.fillTable(new Position());
			}else{
				JOptionPane.showMessageDialog(null, "职位信息添加失败！");
			}
		}catch(Exception e){
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "职位信息添加失败！！");
