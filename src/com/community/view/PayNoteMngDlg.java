...
public class PayNoteMngDlg extends JFrame {
	...
	protected void updateActionPerformed(ActionEvent evt) {
		...
		try {
			...
			if (addNum == 1) {
				JOptionPane.showMessageDialog(null, "收费单信息修改成功！");
				resetValue();
				this.fillTable(new PayNote());
			} else {
				JOptionPane.showMessageDialog(null, "收费单信息修改失败！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "收费单信息修改失败！！");
	...
	protected void addActionPerformed(ActionEvent evt) {
		...
		try {
			...
			if (addNum == 1) {
				JOptionPane.showMessageDialog(null, "收费单信息添加成功！");
				resetValue();
				this.fillTable(new PayNote());
			} else {
				JOptionPane.showMessageDialog(null, "收费单信息添加失败！");
			}
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "收费单信息添加失败！！");
